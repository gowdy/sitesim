...
sqlite3 ${inputName}.sqlite "select (time-1423531206)/3600.,SUM(DoneData/100),group_concat(s.name),group_concat(DoneData/100),group_concat(BandwidthUsed)
from Transfers t
...
sqlite3 ${inputName}.sqlite "select (time-1423531206)/3600.,SUM(DoneData/100),group_concat(s.name),group_concat(DoneData/100)
from Transfers t
left join Links l on (t.[LinkId] == l.[Id])
left join Sites s on (s.[Id] == l.[ToSite])
left join Sites f on (f.Id == l.[FromSite] )
